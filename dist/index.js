import { Client, ActivityType, Partials, PermissionFlagsBits, EmbedBuilder, ButtonBuilder, ButtonStyle, ActionRowBuilder, TextChannel } from 'discord.js';
...
const hasPermission = (user, permission, error) => {
...
const dir = path$1.join(__dirname, "..", "data");
const validate = (store) => {
    ...
    if (!fs$1.existsSync(path$1.join(dir, STORE_NAMES[store]))) {
        fs$1.writeFileSync(path$1.join(dir, STORE_NAMES[store]), "{}");
    }
};
const getAllData = (store) => {
    ...
    try {
        if (!fs$1.existsSync(path$1.join(dir, STORE_NAMES[store])))
            return {};
        return JSON.parse(fs$1.readFileSync(path$1.join(dir, STORE_NAMES[store]), "utf8"));
    }
    catch (e) {
        console.error(e);
    }
...
const setData = (store, key, value) => {
    validate(store);
    try {
        const data = getAllData(store);
        data[key] = value;
        fs$1.writeFileSync(path$1.join(dir, STORE_NAMES[store]), JSON.stringify(data));
    }
    catch (e) {
        console.error(e);
    }
...
const KEYS$1 = Stream("KEYS");
const sendToEach = async (embeds, components, message, callback) => {
    const guildKey = KEYS$1[`${message.guildId}`];
    if (!guildKey)
        return message.reply(`Oops! This server (${message.guild?.name}) doesn't have a key set up!\n\nAdd one with \`${process.env.PREFIX}key set <key>\``);
    const guilds = await client.guilds.fetch();
    for (const _guild of guilds.values()) {
        const guild = await _guild.fetch();
        const key = KEYS$1[`${guild.id}`];
        if (!key)
            continue;
        if (key === guildKey) {
...
const pingToID = (ping) => ping.replace("<@", "").replace(">", "");
const logBan = async (message) => {
    ...
    const row = new ActionRowBuilder().addComponents(cancel, ban);
...
var makeCommand = (command, args, callback) => {
...
makeCommand("help", ["command"], async (message, args) => {
...
${COMMANDS.map((cmd) => `- \`${cmd.name}\``).join("\n")}
    ...
    if (!COMMAND_INFO[command.name])
        return message.reply(`Oops! I don't have any information about that command.`);
    message.reply(`
...
makeCommand("key", ["arg", "value"], async (message, args) => {
    ...
    switch (arg) {
        ...
        case "view": {
            ...
            await message.channel.send(`The key for ${message.guild.name} is \`${decrypt(key)}\`. It is shared with ${shared.length} other server${shared.length === 1 ? "" : "s"}.\n\n${shared
